...
/** Custom SVG label rendered in the center of the donut via Recharts customized label */
...
function CenterLabel({ viewBox, total }: CenterLabelProps) {
  ...
  return (
    <text x={cx} y={cy} textAnchor="middle" dominantBaseline="middle">
      <tspan x={cx} dy="-0.6em" fontSize="11" fill="#6b7280">
        합계
      </tspan>
      <tspan x={cx} dy="1.4em" fontSize="13" fontWeight="600" fill="#111827">
        {formatAmount(total)}
      </tspan>
    </text>
  );
          ...
          <Pie
            ...
            onClick={(entry) => {
              if (entry && entry.label) {
                onSegmentClick('category', entry.label as string);
              }
            }}
